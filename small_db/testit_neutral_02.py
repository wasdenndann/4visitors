...
class DB_act:
    ...
    def anal(self):
        ...
        erg = self.curs.execute('SELECT * FROM personen')
    ...
    def all(self):
        data = self.curs.execute('SELECT * FROM personen')
        self.view(data)
    ...
    def del_set(self):
        ...
        if messagebox.askyesno('Datensatz löschen?', 'Wollen Sie wirklich diesen Datensatz löschen???'):
            ...
            self.curs.execute(("DELETE FROM personen WHERE geb_id=" + id))
            self.conn.commit()
            self.all()
        else:
            return
    ...
    def read_fields():
        ...
        fields = [[base.actpad.var1.get(), base.actpad.var2.get(),
                   base.actpad.var3.get(), base.actpad.var4.get()],
                  [base.actpad.en1.get(), base.actpad.en2.get(),
                   base.actpad.en3.get(), base.actpad.en4.get()],
                  ['geb_id', 'vorname', 'nachname', 'geburtstag']]
        return fields
    ...
    def select_db(self):
        ...
        query = "SELECT * FROM personen WHERE " + where
        data = self.curs.execute(query)
        self.view(data)
        # self.conn()
        print(query)
    ...
    def update_db(self):
        ...
        if len(order) > 1:
            for com in range(1, len(order)):
                where = where + " AND " + order[com]
        query = "UPDATE personen SET " + where + " WHERE " + fields[2][0] + "=" + fields[1][0]
        # data = self.curs.execute(query)
        data = self.curs.execute((query))
        self.view(data)
        self.conn.commit()
        data = self.curs.execute("SELECT * FROM personen WHERE "+ fields[2][0] + "=" + fields[1][0])
        self.view(data)
        print(query)
...
class Table(ttk.Treeview):
    ...
    def __init__(self, container):
        ...
        columns = ('ID', 'Vorname', 'Nachname', 'Geburtstag')
    ...
    def create_view(self):
        ...
        self.column('#2', anchor='e')
        self.heading('#2', text='Vorname', command=partial(self.sort, '#2', False))
        self.column('#3', anchor='w')
        self.heading('#3', text='Nachname', command=partial(self.sort, '#3', False))
        self.column('#4', anchor=tk.CENTER)
        self.heading('#4', text='Geburtstag', command=partial(self.sort, '#4', False))
    ...
    def show_item(self, event):
        ...
        if region != "heading":
            ...
            id, vorname, nachname, geburt = self.item(current_item, 'values')
...
class ActPad(tk.Frame):
    ...
    def labels(self):
        ...
        lab2 = tk.Label(self, text='Vorname')
        lab2.grid(column=1, row=1, sticky='w', padx=5, pady=0)
        lab3 = tk.Label(self, text='Nachname')
        lab3.grid(column=2, row=1, sticky='w', padx=5, pady=0)
        lab4 = tk.Label(self, text='Geburtstag')
